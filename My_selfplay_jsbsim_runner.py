...
class SelfplayJSBSimRunner(JSBSimRunner):
    ...
    def render(self):
        ...
        # 1. 配置参数
        max_episodes = 30  # 目标采集局数
        gc_interval = 5  # 每5局释放一次内存
        # --- 强制指定到项目根目录下的 expert_pkls ---
        # 无论你在哪启动，它都会保存在你代码文件所在的根目录/expert_pkls
        base_path = os.getcwd()
        save_dir = os.path.join(base_path, "expert_pkls")

        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
            print(f"创建目录: {save_dir}")

        # 打印绝对路径，你直接复制这个路径去文件夹搜索
        print(f"!!!! 文件将保存到绝对路径: {os.path.abspath(save_dir)}")
        # 2. 模型准备
        ...
        logging.info(f"\n开始专家轨迹采集，总局数: {max_episodes} ...")

        for ep_idx in range(max_episodes):
            logging.info(f"Episode {ep_idx} 正在运行...")
            render_episode_rewards = 0
            # 初始化本局存储字典
            episode_storage = {"obs": [], "acts": [], "rews": [], "terminal_obs": None}
            ...
            render_obs = render_obs[:, :self.num_agents // 2, ...]  # Ego obs

            render_opponent_masks = np.ones_like(render_masks)
            render_opponent_rnn_states = np.zeros_like(render_rnn_states)

            while True:
                # 【核心对齐】先备份当前时刻的状态 s_t
                curr_ego_obs = render_obs.copy()

                # --- Ego 决策 (强制指定维度为 (1, Obs_Dim)) ---
                self.policy.prep_rollout()
                render_actions_t, render_rnn_states_t = self.policy.act(
                    render_obs.reshape(1, -1),  # 强制 2D
                    render_rnn_states.reshape(1, -1),  # 强制 2D
                    render_masks.reshape(1, -1),  # 强制 2D
                    deterministic=True
                )
                ego_action_numpy = _t2n(render_actions_t).copy()

                # --- 对手决策 (同样强制指定维度) ---
                self.eval_opponent_policy.prep_rollout()
                opp_actions_t, opp_rnn_states_t = self.eval_opponent_policy.act(
                    render_opponent_obs.reshape(1, -1),  # 强制 2D
                    render_opponent_rnn_states.reshape(1, -1),
                    render_opponent_masks.reshape(1, -1),
                    deterministic=True
                )

                # --- 拼接并执行 Step ---
                combined_actions = np.concatenate((
                    np.expand_dims(ego_action_numpy, axis=0),
                    np.expand_dims(_t2n(opp_actions_t), axis=0)
                ), axis=1)

                render_obs_next, render_rewards, render_dones, render_infos = self.envs.step(combined_actions)

                # --- 数据存入 (s_t, a_t) ---
                # 只有在还没 Done 之前，这一步的 s_t 和 a_t 是有效的
                episode_storage["obs"].append(curr_ego_obs.flatten())
                episode_storage["acts"].append(ego_action_numpy.flatten())

                # 累加奖励 (Ego 部分)
                ego_rew = render_rewards[:, :self.num_agents // 2, ...]
                render_episode_rewards += np.sum(ego_rew)

                # --- 结束判定 ---
                if render_dones.all():
                    # 记录终点状态 s_T，它没有对应的 action
                    episode_storage["terminal_obs"] = render_obs_next[:, :self.num_agents // 2, ...].copy().flatten()
                    break

                # --- 变量更新，准备下一帧 ---
                render_obs = render_obs_next[:, :self.num_agents // 2, ...]
                render_opponent_obs = render_obs_next[:, self.num_agents // 2:, ...]
                render_rnn_states = np.expand_dims(_t2n(render_rnn_states_t), axis=0)
                render_opponent_rnn_states = np.expand_dims(_t2n(opp_rnn_states_t), axis=0)

            # --- 局结束：保存 pkl ---
            # --- 局结束：保存 pkl ---
            reward_score = int(render_episode_rewards)
            # 获取本局的总步数 (T)
            episode_length = len(episode_storage["obs"])

            # 文件名格式：expert_ep[局号]_score[得分]_len[长度].pkl
            pkl_name = f"expert_ep{ep_idx}_score{reward_score}_len{episode_length}.pkl"
            save_path = os.path.join(save_dir, pkl_name)

            with open(save_path, 'wb') as f:
                pickle.dump(episode_storage, f)

            logging.info(f"Saved: {pkl_name} | Total Steps: {episode_length}")

            # --- 定期清理内存 ---
            if (ep_idx + 1) % gc_interval == 0:
                torch.cuda.empty_cache()
                gc.collect()

        logging.info(">>> 所有专家数据采集任务已完成！")